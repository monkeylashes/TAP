Shader "Custom/XZGrid"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			// grid overlay xy
			float2 posXY = IN.worldPos.xy / _GridStep;
			float2 fxy = abs(frac(posXY)-.5);
			float2 dfxy = fwidth(posXY) * _GridWidth;
			float2 gxy = smoothstep(-dfxy, dfxy, fxy);
			float gridXY = 1.0 - saturate(gxy.x * gxy.y);
			
			c.rgb = lerp(c.rgb, float3(1,1,1), grid);
			c.rgb = lerp(c.rgb, float3(1,1,1), gridXY);
